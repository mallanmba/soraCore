...
namespace rapid
{
  ...
  TopicPairApp::Printer::printData(PositionSample const& event)
  {
    ...
    if (s_csv > 0) {
      ...
      if (event.values.length() == 36) {
        //6 x 6 covariance
        for (int i = 0; i < event.values.length(); ++ i) {
          cout << ", ";
          if (event.values[i]._d == rapid::RAPID_DOUBLE) {
            cout << setprecision(12) << event.values[i]._u.d;
          }
          else 
            cout << "-";
        }
      ...
      if (s_verbose == 1) {
        ...
        if (event.values.length() == 36) {
          ...
          for (int i = 0; i < event.values.length(); ++i) {
            ...
            if (event.values[i]._d == rapid::RAPID_DOUBLE) {
              cout << setprecision(6) << event.values[i]._u.d;
            }
            else
              cout << "-";
...
int main(int argc, char * argv[])
{
  ...
  if (app.s_csv > 0) {
    std::cout << "#time, x, y, z, r, p, y, cov 6 x6 "<< std::endl;
  }
