
/*
  WARNING: THIS FILE IS AUTO-GENERATED. DO NOT MODIFY.

  This file was generated from VEStopState.idl using "rtiddsgen".
  The rtiddsgen tool is part of the RTI Connext distribution.
  For more information, type 'rtiddsgen -help' at a command shell
  or consult the RTI Connext manual.
*/

#ifndef VEStopStatePlugin_1271843965_h
#define VEStopStatePlugin_1271843965_h

#include "VEStopState.h"

#include "MessagePlugin.h"


#include "BaseTypesPlugin.h"




struct RTICdrStream;

#ifndef pres_typePlugin_h
#include "pres/pres_typePlugin.h"
#endif


#if (defined(RTI_WIN32) || defined (RTI_WINCE)) && defined(NDDS_USER_DLL_EXPORT)
/* If the code is building on Windows, start exporting symbols.
*/
#undef NDDSUSERDllExport
#define NDDSUSERDllExport __declspec(dllexport)
#endif


namespace rapid{

namespace ext{

/* The type used to store keys for instances of type struct
 * VEStopState.
 *
 * By default, this type is struct VEStopState
 * itself. However, if for some reason this choice is not practical for your
 * system (e.g. if sizeof(struct VEStopState)
 * is very large), you may redefine this typedef in terms of another type of
 * your choosing. HOWEVER, if you define the KeyHolder type to be something
 * other than struct VEStopState, the
 * following restriction applies: the key of struct
 * VEStopState must consist of a
 * single field of your redefined KeyHolder type and that field must be the
 * first field in struct VEStopState.
*/
typedef  class VEStopState VEStopStateKeyHolder;


#define VEStopStatePlugin_get_sample PRESTypePluginDefaultEndpointData_getSample  
#define VEStopStatePlugin_get_buffer PRESTypePluginDefaultEndpointData_getBuffer 
#define VEStopStatePlugin_return_buffer PRESTypePluginDefaultEndpointData_returnBuffer 

#define VEStopStatePlugin_get_key PRESTypePluginDefaultEndpointData_getKey 
#define VEStopStatePlugin_return_key PRESTypePluginDefaultEndpointData_returnKey
 

#define VEStopStatePlugin_create_sample PRESTypePluginDefaultEndpointData_createSample 
#define VEStopStatePlugin_destroy_sample PRESTypePluginDefaultEndpointData_deleteSample 

/* --------------------------------------------------------------------------------------
    Support functions:
 * -------------------------------------------------------------------------------------- */

NDDSUSERDllExport extern VEStopState*
VEStopStatePluginSupport_create_data_w_params(
    const struct DDS_TypeAllocationParams_t * alloc_params);

NDDSUSERDllExport extern VEStopState*
VEStopStatePluginSupport_create_data_ex(RTIBool allocate_pointers);

NDDSUSERDllExport extern VEStopState*
VEStopStatePluginSupport_create_data(void);

NDDSUSERDllExport extern RTIBool 
VEStopStatePluginSupport_copy_data(
    VEStopState *out,
    const VEStopState *in);

NDDSUSERDllExport extern void 
VEStopStatePluginSupport_destroy_data_w_params(
    VEStopState *sample,
    const struct DDS_TypeDeallocationParams_t * dealloc_params);

NDDSUSERDllExport extern void 
VEStopStatePluginSupport_destroy_data_ex(
    VEStopState *sample,RTIBool deallocate_pointers);

NDDSUSERDllExport extern void 
VEStopStatePluginSupport_destroy_data(
    VEStopState *sample);

NDDSUSERDllExport extern void 
VEStopStatePluginSupport_print_data(
    const VEStopState *sample,
    const char *desc,
    unsigned int indent);


NDDSUSERDllExport extern VEStopState*
VEStopStatePluginSupport_create_key_ex(RTIBool allocate_pointers);

NDDSUSERDllExport extern VEStopState*
VEStopStatePluginSupport_create_key(void);

NDDSUSERDllExport extern void 
VEStopStatePluginSupport_destroy_key_ex(
    VEStopStateKeyHolder *key,RTIBool deallocate_pointers);

NDDSUSERDllExport extern void 
VEStopStatePluginSupport_destroy_key(
    VEStopStateKeyHolder *key);

/* ----------------------------------------------------------------------------
    Callback functions:
 * ---------------------------------------------------------------------------- */

NDDSUSERDllExport extern PRESTypePluginParticipantData 
VEStopStatePlugin_on_participant_attached(
    void *registration_data, 
    const struct PRESTypePluginParticipantInfo *participant_info,
    RTIBool top_level_registration, 
    void *container_plugin_context,
    RTICdrTypeCode *typeCode);

NDDSUSERDllExport extern void 
VEStopStatePlugin_on_participant_detached(
    PRESTypePluginParticipantData participant_data);
    
NDDSUSERDllExport extern PRESTypePluginEndpointData 
VEStopStatePlugin_on_endpoint_attached(
    PRESTypePluginParticipantData participant_data,
    const struct PRESTypePluginEndpointInfo *endpoint_info,
    RTIBool top_level_registration, 
    void *container_plugin_context);

NDDSUSERDllExport extern void 
VEStopStatePlugin_on_endpoint_detached(
    PRESTypePluginEndpointData endpoint_data);
    
NDDSUSERDllExport extern void    
VEStopStatePlugin_return_sample(
    PRESTypePluginEndpointData endpoint_data,
    VEStopState *sample,
    void *handle);    
 

NDDSUSERDllExport extern RTIBool 
VEStopStatePlugin_copy_sample(
    PRESTypePluginEndpointData endpoint_data,
    VEStopState *out,
    const VEStopState *in);

/* --------------------------------------------------------------------------------------
    (De)Serialize functions:
 * -------------------------------------------------------------------------------------- */

NDDSUSERDllExport extern RTIBool 
VEStopStatePlugin_serialize(
    PRESTypePluginEndpointData endpoint_data,
    const VEStopState *sample,
    struct RTICdrStream *stream, 
    RTIBool serialize_encapsulation,
    RTIEncapsulationId encapsulation_id,
    RTIBool serialize_sample, 
    void *endpoint_plugin_qos);

NDDSUSERDllExport extern RTIBool 
VEStopStatePlugin_deserialize_sample(
    PRESTypePluginEndpointData endpoint_data,
    VEStopState *sample, 
    struct RTICdrStream *stream,
    RTIBool deserialize_encapsulation,
    RTIBool deserialize_sample, 
    void *endpoint_plugin_qos);

 
NDDSUSERDllExport extern RTIBool 
VEStopStatePlugin_deserialize(
    PRESTypePluginEndpointData endpoint_data,
    VEStopState **sample, 
    RTIBool * drop_sample,
    struct RTICdrStream *stream,
    RTIBool deserialize_encapsulation,
    RTIBool deserialize_sample, 
    void *endpoint_plugin_qos);




NDDSUSERDllExport extern RTIBool
VEStopStatePlugin_skip(
    PRESTypePluginEndpointData endpoint_data,
    struct RTICdrStream *stream, 
    RTIBool skip_encapsulation,  
    RTIBool skip_sample, 
    void *endpoint_plugin_qos);

NDDSUSERDllExport extern unsigned int 
VEStopStatePlugin_get_serialized_sample_max_size(
    PRESTypePluginEndpointData endpoint_data,
    RTIBool include_encapsulation,
    RTIEncapsulationId encapsulation_id,
    unsigned int current_alignment);

NDDSUSERDllExport extern unsigned int 
VEStopStatePlugin_get_serialized_sample_min_size(
    PRESTypePluginEndpointData endpoint_data,
    RTIBool include_encapsulation,
    RTIEncapsulationId encapsulation_id,
    unsigned int current_alignment);

NDDSUSERDllExport extern unsigned int
VEStopStatePlugin_get_serialized_sample_size(
    PRESTypePluginEndpointData endpoint_data,
    RTIBool include_encapsulation,
    RTIEncapsulationId encapsulation_id,
    unsigned int current_alignment,
    const VEStopState * sample);



/* --------------------------------------------------------------------------------------
    Key Management functions:
 * -------------------------------------------------------------------------------------- */

NDDSUSERDllExport extern PRESTypePluginKeyKind 
VEStopStatePlugin_get_key_kind(void);

NDDSUSERDllExport extern unsigned int 
VEStopStatePlugin_get_serialized_key_max_size(
    PRESTypePluginEndpointData endpoint_data,
    RTIBool include_encapsulation,
    RTIEncapsulationId encapsulation_id,
    unsigned int current_alignment);

NDDSUSERDllExport extern RTIBool 
VEStopStatePlugin_serialize_key(
    PRESTypePluginEndpointData endpoint_data,
    const VEStopState *sample,
    struct RTICdrStream *stream,
    RTIBool serialize_encapsulation,
    RTIEncapsulationId encapsulation_id,
    RTIBool serialize_key,
    void *endpoint_plugin_qos);

NDDSUSERDllExport extern RTIBool 
VEStopStatePlugin_deserialize_key_sample(
    PRESTypePluginEndpointData endpoint_data,
    VEStopState * sample,
    struct RTICdrStream *stream,
    RTIBool deserialize_encapsulation,
    RTIBool deserialize_key,
    void *endpoint_plugin_qos);

 
NDDSUSERDllExport extern RTIBool 
VEStopStatePlugin_deserialize_key(
    PRESTypePluginEndpointData endpoint_data,
    VEStopState ** sample,
    RTIBool * drop_sample,
    struct RTICdrStream *stream,
    RTIBool deserialize_encapsulation,
    RTIBool deserialize_key,
    void *endpoint_plugin_qos);


NDDSUSERDllExport extern RTIBool
VEStopStatePlugin_serialized_sample_to_key(
    PRESTypePluginEndpointData endpoint_data,
    VEStopState *sample,
    struct RTICdrStream *stream, 
    RTIBool deserialize_encapsulation,  
    RTIBool deserialize_key, 
    void *endpoint_plugin_qos);

 
NDDSUSERDllExport extern RTIBool 
VEStopStatePlugin_instance_to_key(
    PRESTypePluginEndpointData endpoint_data,
    VEStopStateKeyHolder *key, 
    const VEStopState *instance);

NDDSUSERDllExport extern RTIBool 
VEStopStatePlugin_key_to_instance(
    PRESTypePluginEndpointData endpoint_data,
    VEStopState *instance, 
    const VEStopStateKeyHolder *key);

NDDSUSERDllExport extern RTIBool 
VEStopStatePlugin_instance_to_keyhash(
    PRESTypePluginEndpointData endpoint_data,
    DDS_KeyHash_t *keyhash,
    const VEStopState *instance);

NDDSUSERDllExport extern RTIBool 
VEStopStatePlugin_serialized_sample_to_keyhash(
    PRESTypePluginEndpointData endpoint_data,
    struct RTICdrStream *stream, 
    DDS_KeyHash_t *keyhash,
    RTIBool deserialize_encapsulation,
    void *endpoint_plugin_qos); 
     
/* Plugin Functions */
NDDSUSERDllExport extern struct PRESTypePlugin*
VEStopStatePlugin_new(void);

NDDSUSERDllExport extern void
VEStopStatePlugin_delete(struct PRESTypePlugin *);

} /* namespace ext */

} /* namespace rapid */

        
#if (defined(RTI_WIN32) || defined (RTI_WINCE)) && defined(NDDS_USER_DLL_EXPORT)
/* If the code is building on Windows, stop exporting symbols.
*/
#undef NDDSUSERDllExport
#define NDDSUSERDllExport
#endif        

#endif /* VEStopStatePlugin_1271843965_h */
